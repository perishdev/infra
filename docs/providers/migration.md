...
Deep dive for [Phase 5](../wizard.md#phase-5--migration-adopt-existing-resources) of the
...
[`import.md`](../import.md); this file is the cross-provider pattern and the actor split.
...
4. **Plan** — the success signal is *"will be imported"*, and crucially **nothing
   *"will be created"*.**
5. **Commit + apply** on merge (human/API-confirmed, per [`ci.md`](../ci.md)).
...
[`import.md`](../import.md). Don't duplicate it; the agent should read and follow it.
...
([`ci.md`](../ci.md)); a maintainer reads the plan (UI or the
[HCP API toolkit](../hcp-api.md)) and confirms the apply on merge. The import executes as a
