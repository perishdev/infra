...
#
# Imported from existing Cloudflare state via `cf-terraforming` (see
# docs/import.md). Resource labels were renamed for readability — the
# `id` values inside `import` blocks remain the canonical Cloudflare
# record IDs.
#
# Apex / www records (A perish.dev, A www.perish.dev) are intentionally
# absent from this file. They currently point at the IANA documentation
# IP 192.0.2.1 (unroutable) with a Cloudflare Redirect Rule serving
# perish.dev -> github.com/perishdev at the edge. The follow-up PR
# replaces them with GitHub Pages records (4× apex A records + www
# CNAME) once perishdev/perishdev.github.io is live.
